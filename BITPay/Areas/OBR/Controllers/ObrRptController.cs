...
namespace BITPay.Areas.OBR.Controllers
{
    ...
    public class ObrRptController : BaseController
    {
        ...
        public ObrRptController(IOptions<AppConfig> appSett,IJsReportMVCService jsReportMVCService)
        {
        ...
        public async Task<IActionResult> Main()
        {
            ...
            var rpt = reports.Where(x => x.ReportTitle == "ObrRpt");
        ...
        public async Task<IActionResult> Generate(ReportGenModel model)
        {
            try
            {
                var reportData = await bl.ProcessObrReportData(model);
        ...
        {
            var list = bl.GetListModel(DBL.Enums.ListModelType.Office).Result.Select(x => new SelectListItem
            {
                Text = x.Text,
                Value = x.Value
            }).ToList();
            ViewData["Office"] = list;

            list = bl.GetReports().Result.Where(x => x.ReportTitle == "ObrRpt").Select(x => new SelectListItem
            {
                Text = x.ReportTitle,
