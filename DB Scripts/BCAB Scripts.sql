...
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('BANK_NAME','BCAB S.A','Bank Name',1)
...
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('CBS_UPLOAD','0','Posting to CBS 0 - Manual,1 - System Upload',1)
If Not Exists(Select * From SysSettings Where ItemName = 'ORACLE_USER')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('ORACLE_USER','orion','Oracle User Name',1)
If Not Exists(Select * From SysSettings Where ItemName = 'ORACLE_PASS')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('ORACLE_PASS','','Oracle password',1)
If Not Exists(Select * From SysSettings Where ItemName = 'ORACLE_HOST')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('ORACLE_HOST','10.10.100.11','Oracle Host',1)
If Not Exists(Select * From SysSettings Where ItemName = 'ORACLE_SID')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('ORACLE_SID','bcab','Oracle SID',1)
If Not Exists(Select * From SysSettings Where ItemName = 'ORACLE_PORT')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('ORACLE_PORT','6680','Port',1)
If Not Exists(Select * From SysSettings Where ItemName = 'ORACLE_SCHEMA')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('ORACLE_SCHEMA','amparr','Schema',1)
If Not Exists(Select * From SysSettings Where ItemName = 'BANK_CODE')
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('BANK_CODE','BCAB','Bank Code',1)
    ...
    BEGIN TRY
		...
		Begin
			...
			Begin
				...
				Return
			End
			Select @TaxAcc= Replace(Replace( Coalesce(''''+ItemValue+ '''', '') +  ',',',',''','''),'''''','''')+''''  from SysSettings where ItemName='TAX_CR_ACCOUNT'
		End
		Else If(@PostToCBS=1)
		Begin
...
GO
INSERT [dbo].[CustomTax] ([TaxNo], [TaxName], [TaxCode]) VALUES (1, N'Payement des déclarations au comptant', 0)
...
If not exists(select * from sys.columns where name=N'Currency' and object_id=object_id(N'dbo.TaxFiles'))	
	Alter Table TaxFiles add   Currency int default(108) not null
...
Select 
	...
	b.StatusCode,
	b.StatusMsg,
	b.Extra1 as OBRReceiptNo,
	b.PayType,
	case b.PayType when 1 then 'PAIEMENT DE DÉCLARATION' when 2 then 'PAIEMENT DE CREDIT'
	when 3 then 'PAIEMENT PAR BORDEREAU DE CRÉDIT' when 4 then 'AUTRES PAIEMENTS' when 6 then 'PAIEMENT DE DÉCLARATION' 
	when 7 then 'PAIEMENT DE CREDIT'else 'AUTRES PAIEMENTS EN VRAC' end as PaymentType
From Payments a
...
BEGIN
    ...
    BEGIN TRY
		---- Validate
		Select @Attempts = NotifAttempts, @FileStat = StatusCode From TaxFiles Where FileCode = @FileCode
		If(@FileStat Is Null)
		Begin
...
BEGIN
    ...
    Begin Try
     ...
     -- if exists(select declCode from TempTaxDets where declarantCode=@declarantCode)
     -- Begin
     -- select 1 as RespStatus, 'Record already exists' as RespMessage;
     -- return;
     -- End
            --- Generate Item code
        ...
        Select @Code = RunNo From @RunNoTable
		   ...
		   ----validate nulls
		   --if((@Code!='' or @Code!=null) and 
		   --(@userCode!='' or @userCode!=null) and 
     --       (@officeCode!=''or @officeCode!='') and
     --       (@officeName!='' or @officeName!=null) and
     --       (@declarantCode!='' or @declarantCode!=null) and
     --       (@declarantName!='' or @declarantName!=null) and
     --       (@companyCode!='' or @companyCode!=null) and
     --       (@companyName!='' or @companyName!=null ) and
     --       (@assessmentYear!='' or @assessmentYear!=null) and
     --       (@assessmentSerial!='' or @assessmentSerial!=null) and
     --       (@assessmentNumber!='' or @assessmentNumber!=null) and
     --       (@RegistrationNumber!='' or @RegistrationNumber!=null) and
     --       (@RegistrationSerial!=''  or @RegistrationSerial!=null) and
     --       (@amountToBePaid!='' or @amountToBePaid!=null))
		...
		--else
		--Begin
		 --  Select  1 as RespStatus, ' TEMP declaration cannot be empty!' as RespMessage
		--return;
		--End
        -- Create response
...
BEGIN
	...
	if(@mode=0)
		Begin
             Select * from TempTaxDets   where usercode=@userCode;
        End
	...
	if(@mode=2)
		Begin
		    select @Data1=sum(amountToBePaid)  from TempTaxDets where usercode=@userCode;
			...
			receiptSerial,receiptNumber,receiptDate,payType from TempTaxDets
...
Select 
   ...
   (select username from users where UserCode=a.Checker)as Checker_Id,
	...
	b.PayType,
	case b.PayType when 1 then 'PAIEMENT DE DÉCLARATION' when 2 then 'PAIEMENT DE CREDIT'
	when 3 then 'PAIEMENT PAR BORDEREAU DE CRÉDIT' when 6 then 'PAIEMENT DE DÉCLARATION EN VRAC' when 7 then 'PAIEMENT DE CREDIT EN VRAC'
	else 'AUTRES PAIEMENTS' end as PaymentType
From Payments a 
Inner join TaxFiles b on a.FileCode = b.FileCode
...
Insert into SysSettings(ItemName,ItemValue,Descr,Editable) values('DOM_BANK_CODE','BCAB','Domestic Bank Code',1)
